---
video-link:
  - https://www.bilibili.com/video/BV1vb421H71y/
---

# Question

```
A

Sharon Brown (11)______ (win) a Women in Innovation award in 2021 for her work in developing a blood test that can help to identify cancer quicker and more accurately.
The award was given by an organisation (12)______ (call) Innovate UK for International Women's Day celebrations.
Sharon is one of 38 women from the UK to have received the award.
The award aims to support brilliant women to (13)______ (full) achieve their visions and change the world in future.

B

Language Days at the United Nations seek (14)______ (celebrate) cultural diversity and promote equal usage of the official languages.
There are six days for each of the six official languages.

The Chinese language is one of the six official languages chosen by the United Nations, and UN Chinese Language Day (15)______ (hold) on April 20 every year.
The date was selected from Guyu, (16)______ is the 6th of 24 solar terms in the traditional Chinese calendar.

C

Self-discipline is essential for every person, regardless of (17)______ (he) age, class or profession.
It shows a person's character and helps him build relationships with those around him.
It also helps in making good use of the available time.
And this (18)______ (result) in increased productivity and enhanced efficiency.
People who practice self-discipline are always respected (19)______ society.

It is proved (20)______ small changes in routine life such as sleeping and waking up at the same time each day, eating healthy food, exercising and setting goals can help in achieving self-discipline.
```

# Answer

## A

```
A

Sharon Brown (11)______ (win) a Women in Innovation award in 2021 for her work in developing a blood test that can help to identify cancer quicker and more accurately.
The award was given by an organisation (12)______ (call) Innovate UK for International Women's Day celebrations.
Sharon is one of 38 women from the UK to have received the award.
The award aims to support brilliant women to (13)______ (full) achieve their visions and change the world in future.
```

## 11 动词作谓语——时态/语态/主语

```markdown
Sharon Brown **won** a Women _in Innovation award_ _in 2021_ _for her work_ in developing a blood test
==that== **can help** to identify cancer quicker and more accurately.

// 谓语
时态：过去
语态：主动 “S赢了一个女人”
主语：单数

过去
一般过去 ✅
过去进行 “正在” 🙅
过去完成 两个过去比先后 🙅

核心词汇
win - won - won - winning
```

## 12 动词作非谓语——主动/被动

```markdown
The award **was given** _by an organisation_ _called_ Innovate UK for International Women's Day celebrations.

// 非谓语
“被叫做”

call A B “把A称作B”（双宾语动词）
call him my friend “把他称作我的朋友”

tell sb sth; call A B 双宾语动词如果后面只有一个名词，需要使用被动
```

## 13 词性变换

```markdown
The award **aims** _to support_ brilliant women _to fully achieve_ their visions ==and== _change_ the world in future.

形容词
1 比较级/最高级 🙅 比较的双方/多方
2 词性变换 adv修饰v ✅
```

```
形容词变副词 -ly
特殊变化
全部 真正 相当 负责的 温柔
可能 简单的 不可思议的 可怕

full - fully // ✅ 加y // fullly 🙅
whole - wholly // ✅ 去e加ly

// real - really // 标准变化
true - truly // ✅ 去e加ly

// quite - quitely // 标准变化
considerable - considerably // ✅ 去e加y // ble / tle / ple

responsible - responsibly // ✅ 去e加y

gentle - gently // ✅ 去e加y

probable - probably // ✅ 去e加y

simple - simply // ✅ 去e加y

incredible - incredibly // ✅ 去e加y

horrible - horribly // ✅ 去e加y
terrible - terribly // ✅ 去e加y

有些词不在这里，但格式就很像，以此类推
remarkable - remarkably // ✅ 去e加y
```

## B

```
B

Language Days at the United Nations seek (14)______ (celebrate) cultural diversity and promote equal usage of the official languages.
There are six days for each of the six official languages.

The Chinese language is one of the six official languages chosen by the United Nations, and UN Chinese Language Day (15)______ (hold) on April 20 every year.
The date was selected from Guyu, (16)______ is the 6th of 24 solar terms in the traditional Chinese calendar.
```

## 14 动词作非谓语——主动被动

```markdown
Language Days _at the United Nations_ **seek** _to celebrate_ cultural diversity ==and== _promote_ equal usage of the official languages.

核心词汇
celebrate v.
celebration n.

分析句子结构
动词 空格 名词
空格是名词 🙅 后面有名词
空格是动词作非谓语 ✅

主动 “为了”
seek to do sth 寻求做某事、目标是做某事

to do sth and (to) do sth =
_to do_ sth and _do_ sth 两个非谓语并列（能够反推出to do）

doing sth and to do sth 很怪异 机率很低
```

## 15 动词作谓语——时态/语态/主语

```markdown
The Chinese language **is** one _of the six official languages_ chosen by the United Nations,
==and== UN Chinese Language Day **is held** on April 20 _every year_.

分析句子结构
and并列句子

时态：现在（一般现在）
语态：被动“节日被举办”
主语：单数
```

## 16 无提示词后面是句子：从句引导词

```markdown
The date **was selected** _from Guyu_,
==which== **is** the 6th of 24 solar terms _in the traditional Chinese calendar_.

被动语态的句子是完整句

主v从x 定语从句
物 which // 逗号不能用that
```

## C

```
C

Self-discipline is essential for every person, regardless of (17)______ (he) age, class or profession.
It shows a person's character and helps him build relationships with those around him.
It also helps in making good use of the available time.
And this (18)______ (result) in increased productivity and enhanced efficiency.
People who practice self-discipline are always respected (19)______ society.

It is proved (20)______ small changes in routine life such as sleeping and waking up at the same time each day, eating healthy food, exercising and setting goals can help in achieving self-discipline.
```

## 17 代词形式的变换

```markdown
Self-discipline **is** essential for every person, regardless _of his age_, class or profession.

介词 形容词 名词
```

| 主格 | 宾格    | 自己         | 某人的  | 某人的东西 |
| ---- | ------- | ------------ | ------- | ---------- |
| I    | me      | myself       | my      | mine       |
| you  | you     | yourself     | your    | yours      |
| he   | **him** | himself      | **his** | **his**    |
| she  | **her** | herself      | her     | hers       |
| it   | it      | itself       | **its** | **its**    |
|      |         |              |         |            |
| we   | us      | _ourselves_  | our     | ours       |
| you  | you     | _yourselves_ | your    | yours      |
| they | them    | _themselves_ | their   | theirs     |

- 题目给的提示词一般是“主格”
- 前后不变的基本不会考察
- “某人的东西”考察的概率很低

## 18 动词作谓语——时态/语态/主语

```markdown
It **also helps** in making good use of the available time.
And this **results** in _increased productivity_ and _enhanced efficiency_.

名词 🙅
动词谓语 ✅

时态：现在（一般现在）
语态：主动 “这导致结果” // 如果动词后面不需要有名词，该动作一定为主动
主语：单数

核心词汇
A results in B “A的结果是B”
B results from A “B的结果来自A” “B的原因是A”
My efforts **resulted in** an excellent score in the test.
My excellent score **resulted from** my constant efforts.
```

## 19 无提示词后面是短语：介词

```markdown
People ==who== **practice** self-discipline
**are always respected** at society.

主v从x 从句缺主语 定语从句

by society 🙅 “被社会尊重” chinglish // 社会是没法尊重你的 能尊重你的只有社会里的人
in society ✅ “在社会里受到尊重”

in + 整体范围、领域、区域

介词题重在积累
做错了，多读两遍，不需要太多纠结
随着阅读增加，介词难题会不攻自破
```

## 20 无提示词后面是句子：从句引导词

```markdown
It **is proved**
==that== small changes _in routine life_
such as _sleeping and waking up at the same time each day_, _eating healthy food_, _exercising_ and _setting goals_ 补充的内容 没那么重要
**can help** _in achieving self-discipline_.

主x从v // 主句被动语态视为完整句
出现形式主语it和形式宾语it，视作不存在
主语从句

不确定情况：how（如何）、why（为什么）、whether（是否）
确定情况：that（不需要翻译，多数时候可以省略）

直接记住
It is proved that ... “某事已经得到了证明”
```

# Review

- 18 min

|     | Correct Answer        | My Answer | Status | Analyze                                                                   |
| --- | --------------------- | --------- | ------ | ------------------------------------------------------------------------- |
| 11  | won                   |           | ✅     |                                                                           |
| 12  | called                |           | ✅     |                                                                           |
| 13  | fully                 |           | ✅     |                                                                           |
| 14  | to celebrate          |           | ✅     |                                                                           |
| 15  | is held               |           | 🙅     | 差最后的临门一脚，已经分析出了“一般现在被动语态”，但最后落笔写 was held； |
| 16  | which                 |           | ✅     |                                                                           |
| 17  | his                   |           | ✅     |                                                                           |
| 18  | results / will result |           | ✅     |                                                                           |
| 19  | in                    |           | 🙅     | at society 觉得很通顺，介词只能积累；                                     |
| 20  | that                  |           | ✅     |                                                                           |
