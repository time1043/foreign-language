---
video-link:
  - https://www.bilibili.com/video/BV1vb421H71y?t=472.2&p=2
---

# Question

```
A

Gratitude is more than just saying "thank you". Gratitude is a deeper appreciation for someone or something. Expressing gratitude makes us feel a positive emotion. Over the past thirty years, there (11)______ (be) many studies showing that writing a gratitude letter to another person offers us an opportunity (12)______ (escape) from negative emotions. Even if we don't share our writing with anyone, the act of completing the exercise alone makes us happier and (13)______ (satisfied) with life. The more we express gratitude, the more positive we feel.

B

When I was in Singapore six years ago, I gave a taxi driver a card (14)______ a specific address on it and asked him to take me there as quickly as possible. When we had almost reached the destination, he circled around the block. His meter read $11, but he took only $10. He explained that he wasn't so familiar with this area. Before getting out of the taxi, I (15)______ (tell) that the ride with the taxi driver is always an important experience (16)______ creates the first impression about this country for a person.

C

When were you born? For most people, this is an easy question. Even those who don't know their exact birthday can usually figure out when they were born to within a few years. Yet the Internet is full of quizzes (17)______ (make) to help you determine in which decade you were born. These are usually based on what (18)______ (happen) in American pop culture at the time you first became aware of it. It sounds foolish. But many people, though (19)______ (complain) about the stupid quizzes, still take them, wondering (20)______ their answers generate their correct birth year.
```

# Answer

## A

```
A

Gratitude is more than just saying "thank you".
Gratitude is a deeper appreciation for someone or something.
Expressing gratitude makes us feel a positive emotion.
Over the past thirty years, there (11)___have been___ (be) many studies showing that writing a gratitude letter to another person offers us an opportunity (12)___to escape___ (escape) from negative emotions.
Even if we don't share our writing with anyone, the act of completing the exercise alone makes us happier and (13)___more satisfied___ (satisfied) with life.
The more we express gratitude, the more positive we feel.
```

## 11 动词作谓语（现在完成/主动/复数）

```markdown
_Over the past thirty years_, there **have been** many studies _showing_

时间：从过去到现在/现在完成 // 显眼的标志，没有就别考虑
语态：主动
主语：复数 many studies

// 时间标志（从过去到现在 现在完成）
over / in + past ... “在过去的...时间里”（从现在倒推）
since + 过去时间 “自从过去某时到现在”
by now “过去到现在为止” up to now, till now
recently “最近”

使用现在完成时，句中必然有表示过去到现在的特殊时间标志
```

## 12 动词作非谓语（主动/目的）

```markdown
==that== writing a gratitude letter _to another person_ **offers** us an opportunity _to escape_ _from negative emotions_.

// 非谓语 -> 主动 -> 目的/预期
主动：to do、doing
被动：done 🙅

doing 同时发生 “一边...一边...”（后面出现的是**同步动作**）
to do 目的 “为了...”（后面出现的是**期待的结果**） ✅

escape后面不应该有名词
如果动词后面不需要有名词，该动作一定为主动

“我打他”
“他被我打” // 被动就是 把动词后面的名词 挪到主语的位置

“我逃跑” // 动词后面没有名词可以挪
```

## 13 形容词副词——比较级最高级

```markdown
==Even if== we **don't share** our writing _with anyone_,
the act of completing the exercise alone **makes** us happier ==and== _more satisfied_ with life.

and并列短语（and后面是短语）
and并列 "happier" "more satisfied"

比较级：比较**双方**差异、体现前后变化
最高级：在比较范围**多方**中最特别的

// 暂时忽略语法名词：单音节双音节多音节 音节音标重音
长得很简单的形容词副词：比较级“+er”；最高级“+est”
长得很复杂的形容词副词：比较级“more + 原形”；最高级“the most + 原形”
绝对不存在“more +er”，只能二选一
```

## B

```
B

When I was in Singapore six years ago, I gave a taxi driver a card (14)___with___ a specific address on it and asked him to take me there as quickly as possible.
When we had almost reached the destination, he circled around the block.
His meter read $11, but he took only $10.
He explained that he wasn't so familiar with this area.
Before getting out of the taxi, I (15)___was told___ (tell) that the ride with the taxi driver is always an important experience (16)___that / which___ creates the first impression about this country for a person.
```

## 14 无提示词后面是短语：介词

```markdown
==When== I was in Singapore _six years ago_,
I **gave** a taxi driver a card
==with== a specific address on it
==and== **asked** him to take me there as quickly as possible.

// 句子结构分析
空格后面是名词 -> and后面是动词 并列前面的动词
a taxi driver a card 连续两个名词 😷 双宾语

a card xxx a specific address 那张卡片上“有”一个具体的地址
with 伴随 在一起 有
介词重在积累

The kid has confidence.  
With confidence, the kid do sth. // 用with 不用has 可以用动词干别的事情了
// 写作思路
能用低级别解决的，就不要用高级的；
能用介词解决的，就不要用动词；
能用词解决的，就不要用短语；
能用短语解决的，就不要用句子；
能用一句话解决的，就不要一段话；
```

## 15 动词作谓语（一般过去/被动/单数）

```markdown
==Before== _getting_ out of the taxi,
I **was told**

时态：过去 “走出出租车”（上下文）
语态：被动 “我被告诉” // 如果动词后面应该有名词，但是句中没有，那么该动词很可能是被动
主语：单数

// 过去
一般过去 表示过去普遍情况
过去进行 表示过去某时刻正在发生
过去完成 表示在过去之前发生并完成

过去完成，要求同一句话里，有两个过去，强调先后顺序 // 显眼的标志，没有就别考虑
```

## 16 无提示词后面是句子：从句引导词（主v从x）

```markdown
==that== the ride _with the taxi driver_ **is** always an important experience
==that / which== **creates** the first impression _about this country_ _for a person_.

主v从x 从句缺主语 定语从句
物 an important experience
```

## C

```
C

When were you born?
For most people, this is an easy question.
Even those who don't know their exact birthday can usually figure out when they were born to within a few years.
Yet the Internet is full of quizzes (17)___made___ (make) to help you determine in which decade you were born.
These are usually based on what (18)___was happening___ (happen) in American pop culture at the time you first became aware of it.
It sounds foolish.
But many people, though (19)___complaining___ (complain) about the stupid quizzes, still take them, wondering (20)___whether___ their answers generate their correct birth year.
```

## 17 动词作非谓语（被动）

```markdown
Yet the Internet **is** full of quizzes _made_ _to help_ you determine
==in which== decade you **were born**.

被动 // 如果动词后面应该有名词，但是句中没有，那么该动词很可能是被动
// 整个句子的意思不带看的，这个句子其实不好理解

提示词是动词：考察谓语、考察非谓语、考察词性变化
```

## 18 动词作谓语（过去进行/主动/单数）

```markdown
These **are usually based** on
==what== **was happening** in American pop culture _at the time_
(that) you first **became** aware of it.

time后面的从句，往往省略关系词

时态：过去
语态：主动 “事情发生”
主语：单数（what作主语）

过去进行 _at the time_ (that) you first **became** aware of it
过去进行的被动考的特别少 was/were being done
```

## 19 动词作非谓语（主动/伴随）

```markdown
==But== many people,
==though== **complaining** _about the stupid quizzes_,
**still take** them,
_wondering_
==whether== their answers **generate** their correct birth year.

缺动词？though后句子没主语 🙅
缺名词 ✅

when / while / before / after / though / if 如果后面没有主语，直接出现动词，该动词使用非谓语形式
其实是个省略

“抱怨某人” = “向某人抱怨”（后面不需要名词）
“被抱怨” 🙅

complain v. - complaining, complained
complaint n. - complaints
```

## 20 无提示词后面是句子：从句引导词（主x从v）

```markdown
主x从v 宾语从句
how 如何
why 为什么
whether 是否 “测试结果是不是能.../准不准”

whether和if都可以表示“是否”
只能用whether不能用if：句首、介词后面...
能用if的都能用whether
表示“是否”永远用whether，if只用于表示“如果” // 考试
```

# Review

- 15 min

|     | Correct Answer | My Answer      | Status | Analyze                                                                              |
| --- | -------------- | -------------- | ------ | ------------------------------------------------------------------------------------ |
| 11  | have been      | were           | 🙅     | 分析出了“过去”，但是“There be”句型根深蒂固，只记得“There is/are”和“There was/were”； |
| 12  | to escape      | to escape      | ✅     |                                                                                      |
| 13  | more satisfied | more satisfied | ✅     |                                                                                      |
| 14  | with           | that           | 🙅     | 句子结构好奇怪，有点被吓到，题目链条分析从一开始就错误；                             |
| 15  | was told       | told           | 🙅     | 分析出了“过去”，应该可以分析到“被动”的，但最终失败；                                 |
| 16  | that / which   | that           | ✅     |                                                                                      |
| 17  | made           | making         | 🙅     | 分析出了“非谓语动词”，应该可以分析到“被动”的，但最终失败；                           |
| 18  | was happening  | happened       | 🙅     | 分析出了“过去”，有特别显眼的“过去进行”标志没注意到；                                 |
| 19  | complaining    | complains      | 🙅     | 句子结构复杂，没分析好；<br>对核心词汇 complain 动词名词不熟悉；                     |
| 20  | whether / if   | and            | 🙅     | 句子结构好奇怪，有点被吓到，题目链条分析从一开始就错误；                             |

# 思路总结

- https://www.bilibili.com/video/BV1vb421H71y?t=82.1&p=3

## 解题思路

- 提示词是动词时，主要考查谓语、非谓语、词性变换。
- 如果动词后面不需要接名词，该动作一定为主动。
- 使用现在完成时，句中必然有表示“从过去到现在”的特殊时间标志。
- 使用过去完成时，要求同一句话里有两个过去动作，并强调先后顺序。

## 语法知识

- time 后面的从句往往省略关系词。
- 表示“是否”只用 whether，if 只用于表示“如果”。
- when / while / before / after / though / if 如果后面没有主语，直接出现动词，该动词使用非谓语形式。
- 可以加 -er / -est，也可以用 more / most + 原形，绝对没有 more + 比较级（如 more better）这种形式。

# 考点总结

## 无提示词空格后面是短语——冠词、介词

- 冠词判断空格后面名词是否是独一无二的；
- 介词重点积累常见用法

## 无提示词空格后面是句子——从句关系词、连词

- 观察前后两句话是否完整，是否缺少名词
- 主句不完整、从句完整：how / why / whether
- 主句完整、从句不完整：从句缺人 that / who；缺物 that / which

## 动词作谓语——判断时态语态

- 过去现在将来、主动被动、主语单复数

## 动词作非谓语——判断主动或被动

- 主动：doing 同时发生动作、to do 目的预期结果；
- 被动：done

## 形容词副词的比较级和最高级

- 简单词：+er / +est；
- 复杂词：more / most + 原形

## 词性变换——动名形副之间转换

- 词汇积累
